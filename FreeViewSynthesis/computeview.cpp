...
void computeView::computeProjectionMatrices()
{
    ...
    inMat = ld->curView_CalibParams->m_K;

    //the extrinsic matrix;
    for(i = 0; i < 3; i ++)
    {
        for(j = 0; j < 3; j ++)
            exMat[i][j] = ld->curView_CalibParams->m_Rotmatrix[i][j];
    }

    for(i = 0; i < 3; i ++)
        exMat[i][3] = ld->curView_CalibParams->m_Trans[i];

        //Multiply the in and ex matrix to projection
    for(i = 0; i < 3; i ++)
        for(j = 0; j < 4; j ++)
        {
            ld->curView_CalibParams->m_ProjMatrix[i][j] = 0.0;

            for(k = 0; k < 3; k ++)
                ld->curView_CalibParams->m_ProjMatrix[i][j] += inMat[j][k] * exMat[k][j];
        }

     ld->curView_CalibParams->m_ProjMatrix[3][0] = 0.0;
     ld->curView_CalibParams->m_ProjMatrix[3][1] = 0.0;
     ld->curView_CalibParams->m_ProjMatrix[3][2] = 0.0;
     ld->curView_CalibParams->m_ProjMatrix[3][3] = 1.0;
}
...
void computeView::pointProject_from_ref_to_otherView(int ref, int u, int v, unsigned char d)
{
    double x, y, z = DepthLevelToZ(d);

    projUVZtoXY(ld->m_CalibParams[ref].m_ProjMatrix, (double)u, (double)v, z, &x, &y);

    double *pt = ld->pts;
    pt[2] = projXYZtoUV(ld->curView_CalibParams->m_ProjMatrix, x, y, z, &pt[0], &pt[1]);
    pt[2] = ZToDepthLevel(pt[2]);

}

void computeView::warpingImage(int ref, int cam_id, int proj, cv::Mat &imageColorOut, cv::Mat &imageDepthOut)
{
    for (int v = 0; v < ld->frame_imgArr[ref].rows; v++)
        for (int u = 0; u < ld->frame_imgArr[ref].cols; u++)
        {
            ...
            pointProject_from_ref_to_otherView(cam_id, u, v, d);
            int u1 = (int)ld->pts[0];
            int v1 = (int)ld->pts[1];
            int k1 = (int)ld->pts[2];
